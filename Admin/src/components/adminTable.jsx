...
const AdminTable = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await fetchDataFromApi("/api/admin/all-admins");
        setUsers(res || []);
      } catch (error) {
  ...
  const totalPages = Math.ceil(filteredUsers.length / ADMINS_PER_PAGE);
      ...
      <div className="flex items-center justify-between flex-wrap p-3">
        ...
        <div className="flex items-center">
          <div className="rounded-full h-10 w-80 border-2 border-black flex items-center px-2">
            <input
              className="h-8 w-80 px-2 rounded-full focus:outline-none"
              type="text"
          ...
          <tbody>
            {paginatedUsers.length > 0 ? (
              paginatedUsers.map((user) => (
                <tr key={user._id} className="bg-white border-b-2 border-gray-300">
                  <td className="p-4">
