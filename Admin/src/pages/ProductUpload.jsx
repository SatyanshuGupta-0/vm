...
export default function ProductUpload() {
  ...
  useEffect(() => {
    setProduct((prev) => ({
      ...prev,
      discount: prev.oldPrice - prev.basePrice,
    }));
  ...
  // const handleDeleteVariantImage = async (variantIndex, imgIndex) => {
  //   const updated = [...product.variantOptions];
  //   const imageToDelete = updated[variantIndex].color.images[imgIndex];

  //   // Remove from local state first
  //   updated[variantIndex].color.images.splice(imgIndex, 1);
  ...
  //   e.preventDefault();
  //   try {
  //     await postData("/api/product/create", product,true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await postData("/api/product/create", updatedProduct, true);
  ...
  const handleDeleteModel = async () => {
    const publicId = product.modelPublicId;
    if (!publicId) {
      setProduct((prev) => ({ ...prev, modelLink: "", modelPublicId: "", modelName: "" }));
      return;
    }
    ...
    try {
      ...
      alert("Failed to delete model from Cloudinary");
    }
  };
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-4 overflow-x-auto mt-4">
          {(product.images || []).map((img, idx) => (
            <div key={idx} className="relative w-32 h-32 flex-shrink-0">
              ...
              <button
                type="button"
                onClick={() => handleDeleteImage(img.public_id, idx)}
                className="absolute top-1 right-1 text-white bg-black bg-opacity-75 rounded-full px-2 hover:bg-red-600 transition"
              >
                ✕
              </button>
            </div>
        ...
        {/* Upload Button */}
        {/* <div className="mt-6">
          <label className="block font-medium mb-2">Upload 3D Model (GLB, FBX, etc.)</label>
          <input
            type="file"
        ...
        {/* Show Uploaded Model */}
        {product.modelLink && (
          <div className="mt-4 p-4 bg-gray-100 border rounded flex justify-between items-start sm:items-center gap-4">
            <div className="text-sm text-gray-700 break-all">
              <p className="font-semibold">3D Model:</p>
              <a
                href={product.modelLink}
                target="_blank"
